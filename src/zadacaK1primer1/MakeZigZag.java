...
public class MakeZigZag {
    ...
    public static void makeZigZag(SLL<Integer> list) {
        ...
        SLLNode<Integer> second = list.getFirst().succ;

        while (second != null) {
            if (first.element > 0 && second.element > 0) {
                list.delete(second);
                second = first.succ;
            } else if (first.element < 0 && second.element < 0) {
                list.insertAfter(-first.element, first);
                first = second;
                second = second.succ;
            } else if (first.element == 0) {
                list.delete(first);
                first = first.succ;
                second = second.succ;
            } else {
...
//20
// 0 0 0 -3 -5 -7      4 -3 -6 0 7 8 9 10 7 -2 5 0 0 0
